 ...
 * tracking systems, which parse the PDF this page prints to, and search
 * engines, which can index HTML but barely index a PDF. That constrains the
 * design more than the rest of the site — single column, no CSS grid columns,
 * no images, no icons standing in for words, standard section headings
 * ("Professional Experience", not "Where I've been"), and every link written
 * out as text as well as marked up as an anchor, because a parser that drops
 * the href still needs to read the URL.
 *
 * It lives outside the `(site)` route group on purpose: no intro curtain, no
